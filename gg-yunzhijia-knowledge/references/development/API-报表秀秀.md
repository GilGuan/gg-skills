...
domain: development
module: 报表秀秀
keywords: [IM, OAuth, accessToken, appId, secret]
---
...
访问域名：https://www.yunzhijia.com

接口调用URL：https://域名/接口地址?accessToken=xxxxx，accessToken是访问接口的令牌，每个令牌都只能访问对该令牌授权过的接口
...
url: /gateway/open/linkerp/ReportDataTableapi/getAdminTableInfos
...
备注：1.本接口获取的数据为所有应用管理员报表分类数据，请确保当前账号有查看报表分类权限，否则创建的数据表将无法被当前账号查看到

2.数据表tableInfos内容为通过API接口方式创建，非API接口创建的数据表不在查询之列

2、创建数据表头

url: /gateway/open/linkerp/customizedapi/createTableInfoHeader
...
1.数据表头第一个字段类型必须为string且名称为recordId

3、上传数据表数据

url: /gateway/open/linkerp/customizedapi/saveTableInfoData
...
1.recordId为必录字段且必须唯一，通常为单据编号或自定义唯一主键。当recordId字段与已有字段重复时，对原数据执行更新操作

2.每次上传数据最大数量为500条数据

3.返回结果success为true只说明数据上传接口调用成功，不代表数据已上传成功。数据上传以异步方式进行，需要使用batchId查询上传数据状态

4、查询上传数据表数据上传结果

url: /gateway/open/linkerp/customizedapi/getTableInfoDataSaveStatus
...
url: /gateway/open/linkerp/customizedapi/generateDiagramData
...
1.请确保数据表数据上传结果查询成功后再执行此接口

2.当前接口被调用，当报表编辑中推送规则设置为 “数据更新后立即推送”时会立即推送报表，否则后按照设置的推送时间推送。

6、清空数据表数据

url: /gateway/open/linkerp/customizedapi/clearTableInfoData
